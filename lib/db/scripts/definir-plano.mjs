 ...
 *   node scripts/definir-plano.mjs --familia 3 --plano professional
 *   node scripts/definir-plano.mjs --familia 3 --plano free
 *
...
try {
  if (temFlag("listar") || (!arg("familia") && !arg("plano"))) {
    ...
    if (rows.length === 0) {
      ...
      console.log("");
      console.log("Para mudar:  node scripts/definir-plano.mjs --familia <id> --plano professional");
    }
    process.exit(0);
  }
  ...
  if (familias.length === 0) {
    console.error(`Família ${familiaId} não existe. Use --listar.`);
    process.exit(1);
  }
